...
export default {
  ...
  ssr: {
    ...
    intro: 'PulseアプリケーションのSSRとハイドレーション。',
    quickStart: 'クイックスタート',
    quickStartDesc: 'PulseはHTMLをサーバーでレンダリングし、クライアントでハイドレーションする組み込みSSRを提供します。',
    serverSetup: 'サーバーセットアップ',
    serverSetupDesc: 'Node.js/Expressサーバーで renderToString を使用:',
    clientHydration: 'クライアントハイドレーション',
    clientHydrationDesc: 'クライアントでサーバーレンダリングされたHTMLをハイドレーション:',
    stateSerialization: '状態シリアライゼーション',
    stateSerializationDesc: '状態をサーバーからクライアントに転送:',
    serializeState: '状態をシリアライズ',
    deserializeState: '状態をデシリアライズ',
    restoreState: '状態を復元',
    getSSRState: 'SSR状態を取得',
    modeDetection: 'モード検出',
    modeDetectionDesc: 'SSR中かハイドレーション中かを検出:',
    isSSR: 'SSRモードかチェック',
    isHydrating: 'ハイドレーション中かチェック',
    ssrSafeEffects: 'SSRセーフなエフェクト',
    ssrSafeEffectsDesc: 'エフェクトはSSRモードでサブスクリプションなしで一度実行されます:',
    asyncData: '非同期データ',
    asyncDataDesc: 'useAsyncは自動的にSSRと統合されます:',
    architecture: 'SSRアーキテクチャ',
    architectureDesc: 'SSRシステムを構成するモジュール:',
    module: 'モジュール',
    purpose: '目的',
    ...
    howItWorks: 'SSRの仕組み',
    howItWorksDesc: 'SSRプロセスの各ステップ:',
    step1: 'サーバーレンダリング',
    step1Desc: 'renderToString()が分離されたコンテキストを作成し、MockDOMAdapterを使用',
    step2: '非同期収集',
    step2Desc: '最初のレンダリングでuseAsync操作を収集',
    step3: 'データフェッチ',
    step3Desc: 'すべての非同期操作を待機（タイムアウト付き）',
    step4: '再レンダリング',
    step4Desc: '解決されたデータで2回目のレンダリング',
    step5: 'シリアライゼーション',
    step5Desc: 'MockNodeツリー → HTML文字列',
    step6: '状態転送',
    step6Desc: 'クライアント用に状態をシリアライズ',
    step7: 'ハイドレーション',
    step7Desc: 'クライアントが既存のDOMにリスナーをアタッチ',
    apiReference: 'APIリファレンス',
    renderToString: 'renderToString',
    renderToStringDesc: 'コンポーネントをHTML文字列にレンダリング（非同期、データを待機）',
    renderToStringSync: 'renderToStringSync',
    renderToStringSyncDesc: '同期的にレンダリング（非同期待機なし）',
    hydrate: 'hydrate',
    hydrateDesc: 'サーバーレンダリングされたHTMLをハイドレーション',
    serializeStateApi: 'serializeState',
    serializeStateApiDesc: '状態をシリアライズ（Date、undefined、XSSエスケープを処理）',
    deserializeStateApi: 'deserializeState',
    deserializeStateApiDesc: '状態をデシリアライズ（Dateオブジェクトを復元）',
    fullExample: '完全な例',
    fullExampleDesc: 'SSRを使用した完全なサーバーとクライアントのセットアップ:',
    serverCode: 'サーバーコード',
    clientCode: 'クライアントコード',
    bestPractices: 'ベストプラクティス',
    bestPractice1: 'SSRではブラウザAPIを避ける',
    bestPractice1Desc: 'サーバーコードではwindow、document、localStorageを使用しないでください。isSSR()でチェックしてください。',
    bestPractice2: '非同期操作のタイムアウトを設定',
    bestPractice2Desc: 'ハングリクエストを防ぐために妥当なタイムアウトを設定してください。',
    bestPractice3: '状態シリアライゼーションを処理',
    bestPractice3Desc: 'Dateオブジェクトとundefinedを保持するために提供されたヘルパーを使用してください。',
    bestPractice4: 'ハイドレーションの不一致をテスト',
    bestPractice4Desc: 'サーバーとクライアントのレンダリングが一致することを確認してください。',
    bestPractice5: 'ストリーミングを検討',
    bestPractice5Desc: '大きなページでは、TTFBを改善するためにストリーミングSSRを検討してください。',
    troubleshooting: 'トラブルシューティング',
    issue1: 'ハイドレーション不一致',
    issue1Desc: 'クライアントHTMLがサーバーHTMLと異なります。サーバーとクライアント両方で決定論的なレンダリングを確保してください。',
    issue2: 'window is not defined',
    issue2Desc: 'サーバーコードでブラウザAPIを使用しています。isSSR()チェックでラップしてください。',
    issue3: '非同期データが欠落',
    issue3Desc: 'waitForAsync: trueを確認し、妥当なタイムアウトを設定してください。',
    prevHttp: '← HTTPクライアント',
    nextGraphQL: 'GraphQL →'
  }
};
