...
printSection('Case Conversion Utilities');
...
test('component template uses PascalCase for component name', () => {
  ...
  try {
    ...
    for (const tc of testCases) {
      ...
      assertEqual(pascal, tc.expectedPascal, `PascalCase for "${tc.input}"`);
      ...
      const kebab = tc.input
        ...
        .replace(/^-/, '')
        .replace(/[-_]+/g, '-');
      assertEqual(kebab, tc.expectedKebab, `kebab-case for "${tc.input}"`);
    }
  } finally {
    cleanupTestProject();
  }
...
test('camelCase conversion works correctly', () => {
  ...
  for (const tc of testCases) {
    ...
    assertEqual(result, tc.expected, `camelCase for "${tc.input}"`);
  }
});
...
test('snake_case conversion works correctly', () => {
  ...
  for (const tc of testCases) {
    const result = tc.input
      ...
      .replace(/^_/, '')
      .replace(/[-_]+/g, '_');
    assertEqual(result, tc.expected, `snake_case for "${tc.input}"`);
  }
});
...
printSection('Component Template Generation');
...
`;
  ...
  assert(template.includes('@page Button'), 'Should have @page directive');
  assert(template.includes('state {'), 'Should have state block');
  assert(template.includes('view {'), 'Should have view block');
  assert(template.includes('style {'), 'Should have style block');
  assert(template.includes('.button'), 'Should use kebab-case for class');
});
...
`;
  ...
  assert(template.includes('props {'), 'Should have props block when withProps=true');
  assert(template.includes('title: ""'), 'Should have default props');
  assert(template.includes(`<${pascalName}`), 'Should show usage example');
});
...
test('component template without style option', () => {
  ...
  assertEqual(hasStyle, false, 'Should not include style when withStyle=false');
});
...
printSection('Page Template Generation');
...
view {
  ...
  assert(template.includes('@page DashboardPage'), 'Should have Page suffix');
  assert(template.includes('loading: false'), 'Should have loading state');
  assert(template.includes('actions {'), 'Should have actions block');
  assert(template.includes('async loadData()'), 'Should have loadData action');
  assert(template.includes('.dashboard-page'), 'Should use kebab-case with -page suffix');
});
...
printSection('Store Template Generation');
...
test('store template has correct imports', () => {
  ...
  assert(template.includes('createStore'), 'Should import createStore');
  assert(template.includes('createActions'), 'Should import createActions');
});
...
test('store template has initial state', () => {
  ...
  assert(initialState.includes('items: []'), 'Should have items array');
  assert(initialState.includes('loading: false'), 'Should have loading state');
  assert(initialState.includes('error: null'), 'Should have error state');
});
...
test('store template has CRUD actions', () => {
  ...
  for (const action of actions) {
    assert(template.includes(action), `Should have ${action} action`);
  }
});
...
test('store template has selectors', () => {
  ...
  for (const selector of selectors) {
    assert(selectors.includes(selector), `Should have ${selector} selector`);
  }
});
...
printSection('Hook Template Generation');
...
test('hook template starts with use prefix', () => {
  ...
  assertEqual(hookName, 'useAuth', 'Should add use prefix');
});
...
test('hook template with existing use prefix', () => {
  ...
  assertEqual(hookName, 'useForm', 'Should not duplicate use prefix');
});
...
export function useExample(options = {}) {
  ...
  assert(hookTemplate.includes('pulse('), 'Should use pulse for state');
  assert(hookTemplate.includes('computed('), 'Should use computed for derived values');
  assert(hookTemplate.includes('return {'), 'Should return hook API');
  assert(hookTemplate.includes('options = {}'), 'Should accept options parameter');
});
...
printSection('Service Template Generation');
...
const api = createHttp({
  ...
  assert(template.includes('createHttp'), 'Should import createHttp');
  assert(template.includes('baseURL'), 'Should have baseURL config');
  assert(template.includes('timeout'), 'Should have timeout config');
});
...
test('service template has CRUD methods', () => {
  ...
  for (const method of methods) {
    assert(methods.includes(method), `Should have ${method} method`);
  }
});
...
test('service template methods return promises', () => {
  ...
  assert(template.includes('async'), 'Methods should be async');
  assert(template.includes('await'), 'Methods should use await');
  assert(template.includes('return response.data'), 'Methods should return data');
});
...
import assert from 'node:assert';`;
  ...
  assert(template.includes('node:test'), 'Should use node:test');
  assert(template.includes('node:assert'), 'Should use node:assert');
});
...
test('test template has describe blocks', () => {
  ...
  assert(template.includes("describe('initialization'"), 'Should have initialization tests');
  assert(template.includes("describe('core functionality'"), 'Should have functionality tests');
  assert(template.includes("describe('error handling'"), 'Should have error handling tests');
});
...
printSection('Context Template Generation');
...
import { pulse, computed } from 'pulse-js-framework/runtime';`;
  ...
  assert(template.includes('createContext'), 'Should import createContext');
  assert(template.includes('useContext'), 'Should import useContext');
  assert(template.includes('Provider'), 'Should import Provider');
});
...
test('context template has provider component', () => {
  ...
  assert(template.includes(`${name}Provider`), 'Should have Provider component');
  assert(template.includes('contextValue'), 'Should create context value');
  assert(template.includes('setValue'), 'Should have setValue action');
  assert(template.includes('reset'), 'Should have reset action');
});
...
test('context template has hook', () => {
  ...
  assert(template.includes(`use${name}`), 'Should have custom hook');
  assert(template.includes('useContext'), 'Should use useContext');
  assert(template.includes('console.warn'), 'Should warn if used outside provider');
});
...
test('context template has HOC', () => {
  ...
  assert(template.includes(`with${name}`), 'Should have HOC');
  assert(template.includes('Wrapped'), 'Should return wrapped component');
});
...
printSection('Layout Template Generation');
...
test('layout template has slots', () => {
  ...
  assert(template.includes('slot "header"'), 'Should have header slot');
  assert(template.includes('slot {'), 'Should have default slot');
  assert(template.includes('slot "footer"'), 'Should have footer slot');
});
...
test('layout template has responsive styles', () => {
  ...
  assert(template.includes('@media'), 'Should have media query');
  assert(template.includes('768px'), 'Should target mobile breakpoint');
});
...
actions {
  ...
  assert(template.includes('sidebarOpen: false'), 'Should have sidebar state');
  assert(template.includes('toggleSidebar'), 'Should have toggle action');
});
...
printSection('Scaffold Types Configuration');
...
test('all scaffold types have required properties', () => {
  ...
  for (const [type, config] of Object.entries(SCAFFOLD_TYPES)) {
    assert(config.description, `${type} should have description`);
    assert(config.defaultDir, `${type} should have defaultDir`);
    assert(config.extension, `${type} should have extension`);
  }
});
...
test('scaffold types use correct extensions', () => {
  ...
  for (const type of pulseTypes) {
    ...
    assertEqual(extension, '.pulse', `${type} should use .pulse extension`);
  }

  for (const type of jsTypes) {
    const extension = '.js';
    assertEqual(extension, '.js', `${type} should use .js extension`);
  }
});

test('test scaffold type uses .test.js extension', () => {
  const testExtension = '.test.js';
  assertEqual(testExtension, '.test.js', 'Test files should use .test.js extension');
});
...
printSection('File Path Generation');
...
test('component path uses PascalCase filename', () => {
  ...
  assertEqual(filename, 'UserCard.pulse', 'Component filename should be PascalCase');
});
...
test('store path uses camelCase filename', () => {
  ...
  assertEqual(filename, 'userData.js', 'Store filename should be camelCase');
});
...
test('hook path preserves use prefix', () => {
  ...
  assert(hookName1.startsWith('use'), 'Should preserve existing use prefix');
  assert(hookName2.startsWith('use'), 'Should add use prefix when missing');
});
...
printSection('Template Content Validation');
...
test('templates have valid CSS syntax in style blocks', () => {
  ...
  assertEqual(openBraces, closeBraces, 'Should have matching braces');
});
...
test('templates have valid view block syntax', () => {
  ...
  assert(validView.includes('view {'), 'Should start with view block');
  assert(validView.includes('@click'), 'Should have event handlers');
});
...
test('handles single character names', () => {
  ...
  assertEqual(pascalName, 'X');
  assertEqual(kebabName, 'x');
});
...
test('handles names with multiple consecutive delimiters', () => {
  ...
  assertEqual(cleaned, 'my-test-name');
});
...
test('handles names starting with numbers', () => {
  ...
  assertEqual(pascalName, '3dButton');
});
...
test('handles unicode characters in names', () => {
  ...
  assertEqual(pascalName, 'BoutonFr');
});
