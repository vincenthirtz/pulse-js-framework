...
describe('computeLIS Algorithm Tests', () => {
  ...
  test('computeLIS: mixed array finds correct LIS', () => {
    ...
    for (let i = 1; i < values.length; i++) {
      assert.ok(values[i] > values[i - 1], `LIS should be strictly increasing: ${values}`);
    }
  });
  ...
  test('computeLIS: typical reorder scenario', () => {
    ...
    assert.ok(result.length >= 2, 'Should find LIS of at least length 2');

    // Verify increasing
    const values = result.map(i => [2, 0, 1, 4, 3][i]);
    for (let i = 1; i < values.length; i++) {
      assert.ok(values[i] > values[i - 1], `LIS should be strictly increasing`);
    }
  });
  ...
  test('computeLIS: large array performance', () => {
    ...
    assert.ok(elapsed < 100, `LIS on 1000 elements should be fast, took ${elapsed}ms`);
...
describe('list() Rendering Tests', () => {
  test('list: renders initial items', () => {
    ...
    assert.strictEqual(children[1].tagName.toLowerCase(), 'li');
    assert.strictEqual(children[1].textContent, 'a');
    assert.strictEqual(children[2].textContent, 'b');
    assert.strictEqual(children[3].textContent, 'c');
  });
  ...
  test('list: removes items', () => {
    ...
    assert.strictEqual(lis[1].textContent, 'c');
  });
  ...
  test('list: reorders items efficiently', () => {
    ...
    assert.strictEqual(initialLis[0].textContent, 'first');
    ...
    assert.strictEqual(reorderedLis.length, 3);
    assert.strictEqual(reorderedLis[0].textContent, 'third');
    assert.strictEqual(reorderedLis[1].textContent, 'second');
    assert.strictEqual(reorderedLis[2].textContent, 'first');
  });
  ...
  test('list: uses index as default key', () => {
    ...
    const lis = Array.from(container.querySelectorAll('li'));
    assert.strictEqual(lis[0].textContent, '0:a');
    assert.strictEqual(lis[1].textContent, '1:b');
    assert.strictEqual(lis[2].textContent, '2:c');
  });
  ...
  test('list: handles objects with complex keys', () => {
    ...
    assert.strictEqual(swappedSpans.length, 2);
    assert.strictEqual(swappedSpans[0].textContent, 'Bob');
    assert.strictEqual(swappedSpans[1].textContent, 'Alice');
  });
  ...
  test('list: template can return array of nodes', () => {
    ...
    assert.strictEqual(container.querySelectorAll('dt').length, 2);
    assert.strictEqual(container.querySelectorAll('dd').length, 2);
  });
});
...
describe('list() Edge Cases', () => {
  test('list: handles null/undefined items gracefully', () => {
    ...
    assert.strictEqual(lis[0].textContent, 'null');
    assert.strictEqual(lis[2].textContent, 'undefined');
  });
  ...
  test('list: handles duplicate keys (last wins)', () => {
    ...
    const lis = Array.from(container.querySelectorAll('li'));
    assert.ok(lis.length >= 1, 'Should render at least one item');
  });
