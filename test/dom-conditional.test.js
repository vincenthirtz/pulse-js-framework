...
describe('when() Conditional Rendering Tests', () => {
  test('when: renders thenTemplate when condition is true', () => {
    ...
    assert.strictEqual(container.querySelectorAll('.else').length, 0);
    assert.strictEqual(container.querySelector('.then').textContent, 'visible');
  });
  ...
  test('when: renders elseTemplate when condition is false', () => {
    ...
    assert.strictEqual(container.querySelector('.else').textContent, 'hidden');
  });
  ...
  test('when: works without elseTemplate', () => {
    ...
    assert.strictEqual(container.querySelectorAll('.content').length, 1);

    // Switch to false - should render nothing
    condition.set(false);

    assert.strictEqual(container.querySelectorAll('.content').length, 0);
  });
  ...
  test('when: accepts Pulse directly instead of function', () => {
    ...
    assert.strictEqual(container.querySelector('span').textContent, 'yes');

    condition.set(false);
    assert.strictEqual(container.querySelector('span').textContent, 'no');
  });
  ...
  test('when: accepts static nodes as templates', () => {
    ...
    assert.strictEqual(container.querySelectorAll('.static-then').length, 1);
  });
  ...
  test('when: cleans up old nodes properly', () => {
    ...
    // Initial render
    assert.strictEqual(thenCount, 1);
    assert.strictEqual(elseCount, 0);

    // Switch - should create else, remove then
    condition.set(false);
    assert.strictEqual(thenCount, 1);
    assert.strictEqual(elseCount, 1);
    assert.strictEqual(container.querySelectorAll('.then').length, 0);
    assert.strictEqual(container.querySelectorAll('.else').length, 1);

    // Switch back
    condition.set(true);
    assert.strictEqual(thenCount, 2);
    assert.strictEqual(container.querySelectorAll('.then').length, 1);
    assert.strictEqual(container.querySelectorAll('.else').length, 0);
  });
  ...
  test('when: handles truthy/falsy values', () => {
    ...
    assert.strictEqual(container.querySelectorAll('.truthy').length, 1);

    value.set(0); // falsy
    assert.strictEqual(container.querySelectorAll('.falsy').length, 1);
  ...
  test('when: template can return array of nodes', () => {
    ...
    assert.strictEqual(container.querySelectorAll('span').length, 3);
...
describe('match() Switch Rendering Tests', () => {
  test('match: switches between cases reactively', () => {
    ...
    status.set('error');
    assert.strictEqual(container.querySelectorAll('.success').length, 0);
    assert.strictEqual(container.querySelectorAll('.error').length, 1);
  });
  ...
  test('match: uses default case when no match', () => {
    ...
    assert.strictEqual(container.querySelectorAll('.default').length, 1);
    assert.strictEqual(container.querySelector('.default').textContent, 'Unknown status');
  });
  ...
  test('match: accepts Pulse directly', () => {
    ...
    // Trigger update after marker is mounted
    value.set('a');
    assert.strictEqual(container.querySelector('span').textContent, 'A');

    value.set('b');
    assert.strictEqual(container.querySelector('span').textContent, 'B');
  });
  ...
  test('match: handles string keys matching values', () => {
    ...
    // Trigger update after marker is mounted
    code.set('200');
    assert.strictEqual(container.querySelector('.code-200').textContent, 'OK');

    code.set('404');
    assert.strictEqual(container.querySelector('.code-404').textContent, 'Not Found');
  });
  ...
  test('match: cleans up old nodes on switch', () => {
    ...
    // Trigger first render after marker is mounted
    tab.set('home');
    assert.strictEqual(homeCreated, 1);
    assert.strictEqual(aboutCreated, 0);
    assert.strictEqual(container.querySelectorAll('.home').length, 1);

    tab.set('about');
    assert.strictEqual(homeCreated, 1);
    assert.strictEqual(aboutCreated, 1);
    assert.strictEqual(container.querySelectorAll('.home').length, 0);
    assert.strictEqual(container.querySelectorAll('.about').length, 1);
  });
...
describe('show() Visibility Toggle Tests', () => {
  ...
  test('show: handles truthy/falsy values', () => {
    ...
    assert.strictEqual(element.style.display, ''); // 1 is truthy
  ...
  test('show: element stays in DOM (unlike when)', () => {
    ...
    visible.set(false);
    assert.strictEqual(container.contains(element), true); // Still in DOM, just hidden
...
describe('Conditional Rendering Edge Cases', () => {
  test('when: handles rapid toggles', () => {
    ...
    assert.strictEqual(container.querySelectorAll('.on').length, 0);
    assert.strictEqual(container.querySelectorAll('.off').length, 1);
  });
  ...
  test('match: handles rapid value changes', () => {
    ...
    assert.strictEqual(container.querySelectorAll('.b').length, 1);
    assert.strictEqual(container.querySelectorAll('.a').length, 0);
    assert.strictEqual(container.querySelectorAll('.c').length, 0);
  });
